...
webimPopup.render=function(jsonMsg)
{
    if(jsonMsg)
	{
...
webimPopup.request=function()
{
	...
	scp.id=webimPopup.scriptId;
	scp.src=webimPopup.requestUrl;
	...
	if(scp.readyState)
	{
		scp.onreadystatechange=function()
		{
			...
			{
				webimPopup.dispose(webimPopup.scriptId);
			}
		}
	}	
	else
...
webimPopup.getSender=function(arr)
{
    if(arr && arr.length && arr.length>0)
    {
        ...
        for(var i=0;i<arr.length;i++)
	    {
		    ...
		    l.push('&nbsp;<a href="http://blog.csdn.net/crackcell/archive/2007/10/07/1814528.aspx#" onclick="webimPopup.showChat(\''+ arr[i] +'\');return false;">'+ arr[i] +'</a><br/>');
	    }
	    return l.join('');
    }
}
webimPopup.showChat=function(name)
{
    if(webImForm.hasChatWindow)
    {
        ...
        {
            webImPopup.close(this.chatDivId);
...
webimPopup.checkCookie=function(msgId)
{
    var arrC = document.cookie.split(';');
    for(var i=0;i<arrC.length;i++)
    {
        if(arrC[i].indexOf('IMMsgID_')==0 || arrC[i].indexOf('IMMsgID_')==1)
        {
            if(msgId>parseInt(arrC[i].split('=')[1]))
            {
                return true;
            }
        }
